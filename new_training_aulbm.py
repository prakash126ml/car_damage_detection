# fasterrcnn_train_albumentations.py
...
class CocoDataset(Dataset):
    def __init__(self, root, annFile, transform=None, augment_prob=0.5):
        self.root = root
...
def get_train_transform():
    return A.Compose([
        ...
        A.Rotate(limit=10, p=0.5),  # rotate ±10 degrees
        ToTensorV2(),
    ],
...
# Training loop with LR scheduler and early stopping
...
if __name__ == "__main__":
    # Paths to your COCO dataset folders and annotation json files
    train_dataset = CocoDataset(
        "dataset/train/images",
        "dataset/train/annotations/instances_train.json",
        transform=get_train_transform(),
        augment_prob=0.5,  # 50% chance to augment each image
    )
    val_dataset = CocoDataset(
        "dataset/val/images",
        "dataset/val/annotations/instances_val.json",
    ...
    model = get_model(num_classes=9)  # 8 classes + background

    train_model(model, train_loader, val_loader, device, num_epochs=20, patience=5, top_n=3)

    # Uncomment to visualize predictions on validation images
